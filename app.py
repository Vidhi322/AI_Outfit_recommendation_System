    ...
    for item in item_list:
        ...
        if path_type=="shirt" or path_type=="t-shirt":
            ...
            for shirt in party_list:
                path_party_shirt = os.path.join(party_path, shirt)
...
def signup_result():
    ...
    new_user = User(uname=name, password=generate_password_hash(pwd, method='pbkdf2:sha256'))
...
def fashion_predict():
    ...
    if file is None or file.filename == '':
        return render_template("fashion.html", pred3="Please upload an image")

    # 🚨 2. Read image safely
    file_bytes = np.frombuffer(file.read(), np.uint8)

    if file_bytes.size == 0:
        return render_template("fashion.html", pred3="Empty image file")

    # 🚨 3. Decode image
    img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)

    if img is None:
        return render_template("fashion.html", pred3="Invalid image format")

    # ✅ Save image (same as your logic)
...
def outfit_upload():
    ...
    # ✅ REDIRECT FIX
    flash("The image of your "+color+" "+type+" has been uploaded successfully.")
    return redirect(url_for('user_profile'))
...
def recommend_combination():
    ...
    #print(pred)
    if len(res) == 0:
        return render_template("display_result.html", img=[], message="No recommendations found. Please upload more items.")
...
# C:\Users\jainn\fashion_env\Scripts\Activate.ps1